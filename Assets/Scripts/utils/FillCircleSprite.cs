...
public static class FillCircleSprite
{
    ...
    private static Sprite s_fullCircleCentered;
    ...
    public static Sprite GetFullCircleCentered()
    {
        if (s_fullCircleCentered != null) return s_fullCircleCentered;
        ...
        for (int y = 0; y < size; y++)
            for (int x = 0; x < size; x++)
            {
                ...
                float dy = y - (size - 1) * 0.5f;
                bool inCircle = dx * dx + dy * dy <= r * r;
                tex.SetPixel(x, y, inCircle ? white : clear);
            }

        tex.Apply();
        s_fullCircleCentered = Sprite.Create(tex, new Rect(0, 0, size, size), new Vector2(0.5f, 0.5f));
        return s_fullCircleCentered;
    }
}
